...
interface StepOneProfileProps {
  formData: {
    perfilUsuario?: 'DOADOR' | 'BENEFICIARIO'
  }
  atualizarDados: (data: { perfilUsuario: 'DOADOR' | 'BENEFICIARIO' }) => void
  onContinuar: () => void
}
  ...
  const CardPerfil = ({
    ...
    <button
      ...
      className={`flex flex-col items-start w-full border rounded-xl p-4 text-left shadow-sm transition hover:border-green-700 ${
        selecionado ? 'border-green-700 bg-green-50' : 'border-gray-300 bg-white'
      }`}
    >
    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <CardPerfil
          ...
          selecionado={selecionado === 'BENEFICIARIO'}
        />
      </div>
